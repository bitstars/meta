		...
		try {
			...
			if (!itemWithMeta.isNull(DataParser.ALL_DB_DATA)) {
				return generateItemView(
						...
						metaData.getString(MetaJSONTranslator.CLASS_NAME_STR), "", false);
			}
			...
			for (int i = 0; i < allDbData.length(); i++) {
				...
				if (dept == "") {
					...
					result += "<input type=hidden name="
							+ MetaJSONTranslator.META_TABLE_STR + " value=" + table + " />";
					form = true;
				}
