...
public class MetaValidator {
	...
	public JSONObject updateObject(String oOld, String oNew, boolean tryUpdate)
		...
		if (oOldJSON.isNull(DataParser.ALL_DB_DATA)) {
			...
			oOldDataJSON = oOldJSON.getJSONArray(DataParser.ALL_DB_DATA)
					.getJSONObject(0);
		}
		JSONObject oNewJSON = new JSONObject(oNew);
		JSONObject test = scanJSONObjects(meta, oOldDataJSON, oNewJSON,
	...
	private JSONObject scanJSONObjects(JSONObject meta, JSONObject oOldJSON,
		...
		ArrayList<String> allFields = MetaParser.getAsArrayList(meta,
				MetaJSONTranslator.FIELDS_ALL_STR);
		...
		if (oOldJSON != null) {
			ArrayList<String> typeId = MetaParser.getAsArrayList(meta,
					MetaJSONTranslator.TYPE_ID_STR);
			if (typeId.size() != 0) {
				if (typeId.size() != 1) {
					if (oNewJSON.isNull(typeId.get(0))) {
						showErrorMsg(new MetaMsg().error()
				...
				String str1 = oOldJSON.getString(typeId.get(0));
		...
		// check regex fields
		if (!meta.isNull(MetaJSONTranslator.FIELDS_REGEX_STR)) {
			Object obj = meta.get(MetaJSONTranslator.FIELDS_REGEX_STR);
		...
		if (!meta.isNull(MetaJSONTranslator.FIELDS_COMPLEX_STR)) {
			Object obj = meta.get(MetaJSONTranslator.FIELDS_COMPLEX_STR);
	...
	public MetaValidator setListener(MetaListener listener) {
